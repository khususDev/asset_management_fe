...
import { useToastStore } from '@/stores/toast' // <-- 3. Import store global
...
const uploadToServer = async (file) => {
  ...
  try {
    const response = await axios.post(API_ENDPOINTS.mediaUpload, formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    })

    emit('update:modelValue', response.data.data.url)

    // 5. Panggil toast global saat sukses
    toastStore.showToast('success', response.data.message || 'File successfully uploaded!')
  } catch (err) {
    // UBAH JUGA DI SINI: Memanggil toast kustom saat ERROR
    showToast('error', err.response?.data?.message || 'Error uploading file.')
  } finally {
    loading.value = false
  }
}

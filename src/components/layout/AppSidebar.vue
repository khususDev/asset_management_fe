<template>
  <aside
    :class="[
      'fixed mt-16 flex flex-col lg:mt-0 top-0 px-5 left-0 bg-white dark:bg-gray-900 dark:border-gray-800 text-gray-900 h-screen transition-all duration-300 ease-in-out z-99999 border-r border-gray-200',
      {
        ...
        'translate-x-0 w-[260px]': isMobileOpen,
  ...
  >
    <div class="py-8 flex w-full justify-center">
      <router-link to="/" class="flex items-center justify-center">
        <template v-if="isExpanded || isHovered || isMobileOpen">
          <img
            :src="settingStore.settings.logo_lg || '/images/logo/vue-long.svg'"
            alt="Logo"
            class="max-h-15 object-contain dark:brightness-100"
          />
        </template>
        <template v-else>
          <img
            :src="settingStore.settings.logo_sm || '/images/logo/vue-small.svg'"
            alt="Logo Icon"
              ...
              <li v-for="(item, index) in menuGroup.items" :key="item.name">
                ...
                >
                  ...
                  <ChevronDownIcon
                    ...
                    :class="[
                      ...
                      {
                        'rotate-180 text-primary': isSubmenuOpen(groupIndex, index),
                      },
                    ]"
                  />
                </button>
                ...
                >
                  ...
                  >
                    <ul
                      :class="[
                        'mt-2 space-y-1 ml-6 pl-4 border-l transition-colors duration-300',
                        isSubmenuOpen(groupIndex, index)
                    ...
                    >
                      <li v-for="(subItem, subIndex) in item.subItems" :key="subItem.name">
                        ...
                        <div v-else>
                          <button
                            @click="toggleNested(groupIndex, index, subIndex)"
                            class="menu-dropdown-item w-full flex items-center justify-between cursor-pointer text-gray-500 font-semibold mt-1"
                          >
                            <span>{{ subItem.name }}</span>
                            <ChevronDownIcon
                              class="w-4 h-4 transition-transform duration-200"
                              :class="{
                                'rotate-180 text-primary': isNestedOpen(
                                  groupIndex,
                                  index,
                                  subIndex,
                                  subItem,
                          ...
                          >
                            <ul
                              v-show="isNestedOpen(groupIndex, index, subIndex, subItem)"
                              :class="[
                                'mt-1 space-y-1 ml-6 border-l transition-colors duration-300',
                                isNestedOpen(groupIndex, index, subIndex, subItem)
                                  ? 'border-stroke dark:border-strokedark'
                                  : 'border-stroke dark:border-strokedark',
                              ]"
                            >
                              <li v-for="subSubItem in subItem.subItems" :key="subSubItem.name">
                                <router-link
                                  :to="subSubItem.path"
                                  :class="[
                                    'menu-dropdown-item text-xs pl-4',
                                    {
    ...
    <div
      v-if="isExpanded || isHovered || isMobileOpen"
      class="p-4 text-center text-[10px] text-gray-400 border-t border-stroke dark:border-strokedark mt-auto"
    >
...
const toggleNested = (groupIndex, itemIndex, subIndex) => {
  const key = `${groupIndex}-${itemIndex}-${subIndex}`
  openNestedMenuKey.value = openNestedMenuKey.value === key ? null : key
}

const isNestedOpen = (groupIndex, itemIndex, subIndex, subItem) => {
  const key = `${groupIndex}-${itemIndex}-${subIndex}`

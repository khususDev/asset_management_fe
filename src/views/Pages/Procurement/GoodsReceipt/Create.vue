  ...
  <div class="mx-auto max-w-screen-2xl">
    ...
    <div class="rounded-lg border border-stroke bg-white shadow-default">
      ...
      <div class="border-b px-6 py-5">
        <h3 class="text-xl font-semibold">Goods Receipt</h3>
      </div>
      ...
      <div class="p-6">
        <div class="grid grid-cols-2 gap-6">
          ...
          <div>
            <label class="mb-2 block text-sm font-medium"> Purchase Order </label>
          ...
          <div>
            <label class="mb-2 block text-sm font-medium"> Receipt Date </label>
        ...
        <div v-if="selectedPO" class="mt-8 rounded border border-stroke bg-slate-50">
          <div class="border-b px-5 py-3">
            <h4 class="font-semibold">Purchase Order Information</h4>
          </div>
          ...
          <div class="grid grid-cols-2 gap-8 p-5">
            <div>
              <table class="w-full">
                <tr>
                  <td class="w-40 py-2 font-medium">Vendor</td>
                ...
                <tr>
                  <td class="py-2 font-medium">Contact</td>
            ...
            <div>
              <table class="w-full">
                <tr>
                  <td class="w-40 py-2 font-medium">PO Number</td>
                ...
                <tr>
                  <td class="py-2 font-medium">PO Date</td>
                ...
                <tr>
                  <td class="py-2 font-medium">Expected Delivery</td>
          ...
          <table class="w-full">
            <thead class="bg-slate-100">
              <tr>
                ...
                <th class="border p-3 text-left">Item Description</th>

                <th class="border p-3">UOM</th>

                <th class="border p-3">PO Qty</th>

                <th class="border p-3">Outstanding</th>

                <th class="border p-3">Receive</th>

                <th class="border p-3">Good</th>

                <th class="border p-3">Reject</th>
              ...
              <tr v-for="(item, index) in form.items" :key="item.purchase_order_item_id">
                ...
                <td class="border p-2">
                  <input
                    ...
                    v-model.number="item.accepted_qty"
                    @input="qtyChanged(item)"
        ...
        <div v-if="form.items.length" class="mt-8 rounded border border-stroke">
          <div class="grid grid-cols-2 gap-8 p-6">
            <div>
              <label class="mb-2 block font-medium"> Receipt Notes </label>

              <textarea
                v-model="form.remarks"
                rows="5"
                class="w-full rounded border border-stroke p-3"
                placeholder="Write receipt notes..."
              />
            </div>

            <div>
              <label class="mb-3 block font-medium"> Receipt Status </label>
              <div class="space-y-3">
          ...
          <div class="flex justify-end gap-3 border-t p-5">
            <button
              type="button"
              @click="$router.back()"
              class="rounded border border-stroke px-6 py-2"
            >
              Cancel
            </button>
            <ButtonLoading
              text="Save Goods Receipt"
              loadingText="Saving Goods Receipt..."
              :loading="loading"
              @click="save"
...
const loadPO = async () => {
  try {
    ...
    const response = await axios.get(`${apiUrl}/get-po`, {
      headers: {
...
const selectPO = () => {
  const po = purchaseOrders.value.find((p) => p.id == form.value.purchase_order_id)
  if (!po) {
  ...
  form.value.items = po.items.map((item) => ({
    ...
    received_before_qty: item.quantity - item.outstanding_qty,
    outstanding_qty: item.outstanding_qty,
    received_qty: item.outstanding_qty,
    accepted_qty: item.outstanding_qty,
    rejected_qty: 0,
    replacement_qty: 0,
    remarks: '',
  }))
...
const qtyChanged = (item) => {
  ...
  if (total > item.outstanding_qty) {
    item.accepted_qty = item.outstanding_qty - item.rejected_qty

    if (item.accepted_qty < 0) item.accepted_qty = 0
  }

  item.received_qty = Number(item.accepted_qty) + Number(item.rejected_qty)
}

const validateItems = () => {
  for (const item of form.value.items) {
    item.received_qty = Number(item.received_qty)

    item.accepted_qty = Number(item.accepted_qty)

    item.rejected_qty = Number(item.rejected_qty)

    if (item.received_qty > item.outstanding_qty) {
      alert(`${item.item_description}\nReceive Qty melebihi Outstanding Qty`)
...
const save = async () => {
  ...
  try {
    ...
    showToast('danger', err.response?.data?.message ?? 'Terjadi kesalahan.')
  } finally {
    loading.value = false
  }
}

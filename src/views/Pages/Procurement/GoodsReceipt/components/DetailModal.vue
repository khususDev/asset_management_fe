  ...
  <div v-if="show" class="fixed inset-0 z-99999 flex items-center justify-center bg-black/50">
    <div class="bg-white rounded-lg w-[1100px] max-h-[90vh] overflow-y-auto p-6">
      <div class="flex justify-between mb-6">
        <PageTitle title="Goods Receipt Details" />
      ...
      <div class="grid grid-cols-2 gap-6 mb-8">
        <div>
          <div class="text-xs text-gray-500">GR Number</div>
      ...
      <table class="w-full border">
        <thead class="bg-gray-100">
          <tr>
            ...
            <th class="border p-2">Accepted</th>

            <th class="border p-2">Rejected</th>
      ...
      <div class="mt-8 flex justify-end">
        <button @click="$emit('close')" class="border rounded px-5 py-2">Close</button>
      </div>
    </div>
  </div>
</template>

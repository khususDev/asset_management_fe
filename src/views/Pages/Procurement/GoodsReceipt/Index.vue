    ...
    <div class="mx-auto max-w-screen-2xl">
        ...
        <DataTable :headers="[
            ...
            <template #top-actions>
                <button @click="$router.push({ name: 'opt_goods_receipt.create' })"
            ...
            <tr v-for="gr in grData.data" :key="gr.id" class="border-b dark:border-strokedark">
                ...
                <td class="p-4 text-center">
                    <TableAction :show-post="gr.status === 'DRAFT'" @view="openDetailModal(gr.id)"
                        @post="confirmPostGR(gr.id)" @print="printGR(gr.id)" />
                </td>
            </tr>
        </DataTable>

        <!-- Modal Detail Diletakkan di Luar Tabel -->
        <DetailModal :show="showDetailModal" :goods-receipt="selectedGR" @close="showDetailModal = false" />

        <!-- Modal Konfirmasi Post (Menggunakan ConfirmModal yang Sama) -->
        <ConfirmModal :show="showPostModal" title="Konfirmasi Post Goods Receipt"
            ...
            confirm-text="Ya, Post" type="success" @close="showPostModal = false" @confirm="executePostGR" />
    </div>
</template>
...
const executePostGR = async () => {
    ...
    try {
        const token = localStorage.getItem('token')
        const response = await axios.post(`${apiUrl}/${targetPostId.value}/post`, {}, {
            headers: { Authorization: `Bearer ${token}` }
        })

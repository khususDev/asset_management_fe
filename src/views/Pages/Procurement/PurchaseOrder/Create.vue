    ...
    >
      <div class="p-6.5 flex flex-col gap-6">
        ...
        >
          ...
          <!-- Ubah grid menjadi md:grid-cols-3 -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
            <!-- TAMBAHAN: Input Tanggal PO -->
            <div>
              <label class="mb-2.5 block text-black dark:text-white font-medium"
                >Tanggal PO *</label
              >
            ...
            <div>
              <label class="mb-2.5 block text-black dark:text-white font-medium"
                >Estimasi Kedatangan Barang</label
              >
            ...
            <div class="flex flex-col gap-2 justify-center pt-4">
              ...
              <span class="text-xs text-gray-500">Centang jika total belanja ditambah PPN.</span>
            </div>
          </div>

          <div>
            <label class="mb-2.5 block text-black dark:text-white font-medium"
              >Catatan Tambahan (Notes)</label
            >
            <textarea
              v-model="form.notes"
              rows="2"
              placeholder="Catatan untuk vendor atau internal..."
              ...
              >
                <div class="w-1/2">
                  <input
                    type="text"
                    v-model="item.notes"
                    placeholder="Catatan spesifik item ini..."
                ...
                <div class="text-right">
                  ...
                  <span class="text-md font-bold text-meta-3">{{
        ...
        >
          <div class="flex gap-4">
            <button
              @click="$router.push({ name: 'opt_purchase_order' })"
          ...
          >
            ...
            >
              <span class="font-bold text-sm text-black dark:text-white uppercase tracking-wider"
                >Grand Total:</span
              >
              <span class="text-xl font-bold text-primary ml-auto">{{
...
const form = ref({
  ...
  payment_term_id: '',
  branch_id: '', // Diubah agar sesuai dengan permintaan backend
  order_date: new Date().toISOString().split('T')[0], // Set default ke hari ini
...
const submitForm = async () => {
  ...
  try {
    ...
    const payload = {
      ...form.value,
      is_pkp: form.value.is_taxable ? 1 : 0,
      is_taxable: form.value.is_taxable ? 1 : 0,
    ...
    const errorMsg =
      error.response?.data?.message || 'Gagal menyimpan. Pastikan semua kolom wajib (*) terisi.'

<template>
  <div class="mx-auto max-w-screen-2xl">
    <Breadcrumb
      ...
      :crumbs="['Administration', 'Maintenance', 'Maintenance Types']"
    />

    <DataTable
      :headers="['Code', 'Maintenance Type Name', 'Description', 'Status', 'Action']"
      :from="maintenanceTypesData.from"
      :to="maintenanceTypesData.to"
      :total="maintenanceTypesData.total"
      :search="search"
      :entries="entries"
      @search="search = $event"
    ...
    >
      ...
      <tr
        v-else
        v-for="type in maintenanceTypesData.data"
        :key="type.id"
      ...
      <TableEmpty
        v-if="!isFetching && (!maintenanceTypesData.data || maintenanceTypesData.data.length === 0)"
        :colspan="5"
      />

      <template #pagination>
        <Pagination :links="maintenanceTypesData.links" @change-page="fetchMaintenanceTypes" />
      </template>
    </DataTable>

    <Modal
      :show="showModal"
      :title="isEdit ? 'Edit Tipe Pemeliharaan' : 'Tambah Tipe Pemeliharaan'"
      @close="closeModal"
    >
      <div class="p-6 flex flex-col gap-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Penjelasan detail tipe pemeliharaan..."
...
const maintenanceTypesData = ref({})
const isFetching = ref(false)

const apiUrl = API_ENDPOINTS.mtnType
...
const fetchMaintenanceTypes = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    maintenanceTypesData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data maintenance types', error)
  } finally {
...
const handleSubmit = () => {
  submit(fetchMaintenanceTypes)
}

const handleDelete = () => {
  confirmDelete(fetchMaintenanceTypes)
}
</script>

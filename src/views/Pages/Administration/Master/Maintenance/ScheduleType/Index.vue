<template>
  <div class="mx-auto max-w-screen-2xl">
    <Breadcrumb
      pageTitle="Schedule Types"
      :crumbs="['Administration', 'Maintenance', 'Schedule Types']"
    />

    <DataTable
      :headers="['Code', 'Schedule Name', 'Description', 'Status', 'Action']"
      :from="scheduleTypesData.from"
      :to="scheduleTypesData.to"
      :total="scheduleTypesData.total"
      :search="search"
      :entries="entries"
      @search="search = $event"
    ...
    >
      ...
      <tr
        v-else
        v-for="type in scheduleTypesData.data"
        :key="type.id"
      ...
      <TableEmpty
        v-if="!isFetching && (!scheduleTypesData.data || scheduleTypesData.data.length === 0)"
        :colspan="5"
      />

      <template #pagination>
        <Pagination :links="scheduleTypesData.links" @change-page="fetchScheduleTypes" />
      </template>
    </DataTable>

    <Modal
      :show="showModal"
      :title="isEdit ? 'Edit Tipe Jadwal' : 'Tambah Tipe Jadwal'"
      @close="closeModal"
    >
      <div class="p-6 flex flex-col gap-4">
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Penjelasan detail jadwal..."
...
const scheduleTypesData = ref({})
const isFetching = ref(false)

const apiUrl = API_ENDPOINTS.mtnSchedule
...
const fetchScheduleTypes = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    scheduleTypesData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data schedule types', error)
  } finally {
...
const handleSubmit = () => {
  submit(fetchScheduleTypes)
}

const handleDelete = () => {
  confirmDelete(fetchScheduleTypes)
}
</script>

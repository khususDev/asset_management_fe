<template>
  <div class="mx-auto max-w-screen-2xl">
    <Breadcrumb
      ...
      :crumbs="['Administration', 'Procurement', 'Vendor Types']"
    />

    <DataTable
      :headers="['Code', 'Type Name', 'Description', 'Status', 'Action']"
      :from="vendorTypesData.from"
      :to="vendorTypesData.to"
      :total="vendorTypesData.total"
      :search="search"
      :entries="entries"
      @search="search = $event"
    ...
    >
      ...
      <tr
        v-else
        v-for="type in vendorTypesData.data"
        :key="type.id"
      ...
      <TableEmpty
        v-if="!isFetching && (!vendorTypesData.data || vendorTypesData.data.length === 0)"
        :colspan="5"
      />

      <template #pagination>
        <Pagination :links="vendorTypesData.links" @change-page="fetchVendorTypes" />
      </template>
    </DataTable>

    <Modal
      :show="showModal"
      :title="isEdit ? 'Edit Tipe Vendor' : 'Tambah Tipe Vendor'"
      @close="closeModal"
    >
      <div class="p-6 flex flex-col gap-4">
        ...
        <FormInput
          ...
          placeholder="e.g., Retail, Corporate, E-Commerce"
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Penjelasan detail tipe vendor..."
...
// --- STATE MANAGEMENT ---
const vendorTypesData = ref({})
const isFetching = ref(false)

const apiUrl = API_ENDPOINTS.prcVendor_type
...
// --- FETCH DATA ---
const fetchVendorTypes = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    vendorTypesData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data vendor types', error)
  } finally {
...
const handleSubmit = () => {
  submit(fetchVendorTypes)
}

const handleDelete = () => {
  confirmDelete(fetchVendorTypes)
}
</script>

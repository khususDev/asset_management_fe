<template>
  <div class="mx-auto max-w-screen-2xl">
    <PageTitle title="Master Payment Terms" />

    <DataTable
      :headers="['Code', 'Term Name', 'Description', 'Status', 'Action']"
      :from="paymentTermsData.from"
      :to="paymentTermsData.to"
      :total="paymentTermsData.total"
      :search="search"
      :entries="entries"
      @search="search = $event"
    ...
    >
      ...
      <tr
        v-else
        v-for="term in paymentTermsData.data"
        :key="term.id"
      ...
      >
        ...
        <td
          class="border-r border-stroke px-4 py-5 text-center last:border-r-0 dark:border-strokedark"
        >
          <TableAction show-edit-delete @edit="openEdit(term)" @delete="openDelete(term.id)" />
        </td>
      </tr>

      <TableEmpty
        v-if="!isFetching && (!paymentTermsData.data || paymentTermsData.data.length === 0)"
        :colspan="5"
      />

      <template #pagination>
        <Pagination :links="paymentTermsData.links" @change-page="fetchPaymentTerms" />
      </template>
    </DataTable>
    ...
    >
      <div class="p-6 flex flex-col gap-4">
        <FormInput
          ...
          placeholder="e.g., NET30, COD, DP50"
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Penjelasan detail syarat pembayaran..."
...
const apiUrl = API_ENDPOINTS.prcPayment
...
// --- FETCH DATA ---
const fetchPaymentTerms = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    paymentTermsData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data payment terms', error)
  } finally {
...
const handleSubmit = () => {
  submit(fetchPaymentTerms)
}

const handleDelete = () => {
  confirmDelete(fetchPaymentTerms)
}
</script>

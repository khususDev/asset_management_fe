  ...
  <div class="mx-auto max-w-screen-2xl">
    ...
    >
      ...
      >
        ...
        <td
          class="border-r border-stroke px-4 py-5 text-center font-medium text-primary last:border-r-0 dark:border-strokedark"
        >
          {{ tax.rate }}%
        </td>
        <td
      ...
      <TableEmpty
        v-if="!isFetching && (!taxesData.data || taxesData.data.length === 0)"
        :colspan="6"
      />

      <template #pagination>
        <Pagination :links="taxesData.links" @change-page="fetchTaxes" />
      </template>
    </DataTable>

    <Modal :show="showModal" :title="isEdit ? 'Edit Pajak' : 'Tambah Pajak'" @close="closeModal">
      <div class="p-6 flex flex-col gap-4">
        <FormInput
          ...
          placeholder="e.g., PPN-11, NON-TAX"
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Penjelasan detail pajak..."
...
const apiUrl = API_ENDPOINTS.prcTax
...
// --- FETCH DATA ---
const fetchTaxes = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    taxesData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data taxes', error)
  } finally {

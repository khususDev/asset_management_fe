<template>
  <div class="mx-auto max-w-screen-2xl">
    <Breadcrumb
      pageTitle="License Types"
      :crumbs="['Administration', 'License Master', 'License Types']"
    />

    <DataTable
      :headers="['Code', 'License Type Name', 'Description', 'Status', 'Action']"
      :from="licenseTypesData.from"
      :to="licenseTypesData.to"
      :total="licenseTypesData.total"
      :search="search"
      :entries="entries"
      @search="search = $event"
    ...
    >
      ...
      <tr
        v-else
        v-for="type in licenseTypesData.data"
        :key="type.id"
      ...
      <TableEmpty
        v-if="!isFetching && (!licenseTypesData.data || licenseTypesData.data.length === 0)"
        :colspan="5"
      />

      <template #pagination>
        <Pagination :links="licenseTypesData.links" @change-page="fetchLicenseTypes" />
      </template>
    </DataTable>

    <Modal
      :show="showModal"
      :title="isEdit ? 'Edit Tipe Lisensi' : 'Tambah Tipe Lisensi'"
      @close="closeModal"
    >
      <div class="p-6 flex flex-col gap-4">
        <FormInput
          label="License Type Code"
          v-model="form.code"
          format="uppercase"
          :disabled="capitalize"
          :error="errors?.code ? errors.code[0] : null"
          placeholder="e.g., PER, SUB, OPS"
        />
        ...
        <div>
          ...
          <textarea
            ...
            rows="3"
            format="capitalize"
            placeholder="Detail penjelasan tipe lisensi..."
...
// --- STATE MANAGEMENT ---
const licenseTypesData = ref({})
const isFetching = ref(false)

const apiUrl = API_ENDPOINTS.lcsType
...
// --- FETCH DATA ---
const fetchLicenseTypes = async (url = apiUrl) => {
  isFetching.value = true
  try {
    const response = await axios.get(url, {
      params: { search: search.value, entries: entries.value },
    })
    licenseTypesData.value = response.data.data
  } catch (error) {
    console.error('Gagal load data license types', error)
  } finally {
...
const handleSubmit = () => {
  submit(fetchLicenseTypes)
}

const handleDelete = () => {
  confirmDelete(fetchLicenseTypes)
}
</script>
